...
$answer = $client->createEmailMessage({sender_name=>'Ваня Петров', sender_email=>'uni.sender.gem@gmail.com',
...
$answer = $client->createSmsMessage({sender=>'Tester',
...
$answer = $client->createSmsMessage({sender=>'1 234 4567890',
list_id=>$available_list_ids[-1],body=>"Просто смска with foo message"});
$result = $answer->{result};
isnt ($result, '', 'shoudnt be empty');
is ($answer->{error}, undef, 'shoudnt have errors');
